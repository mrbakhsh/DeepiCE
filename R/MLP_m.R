  ...
  .cm_f <- function(preds, real){
    ...
    preds <- # change character to factor
      as.factor(as.numeric(preds))
    ...
    if (x == 2) {
      preds <- factor(preds,
                              levels = rev(levels(preds))
      )
    }

    real <- # change character to factor
      as.factor(as.numeric(real))
  ...
  #' @return High-confidence interactions defined by user-defined threshold.
  ...
  #' @description This function uses the feedforward deep neural
  ...
  MLP_m <-
    ...
    pred_dat_filt <-
      filter(prob_u,Prob > cutoff) %>%
      separate(PPI, c("p1", "p2"), sep = "~")


    return(pred_dat_filt)
