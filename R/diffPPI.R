  ...
  #' provided by the user. Defaults to NULL.
  ...
  #' All pathways below the threshold are excluded. Defaults to 2.
  ...
  diffPPI <-
    function(m1,
      ...
      fname <- file.path(tpath, "differential_PPI_filt.txt")
      ...
      if(!is.null(term)) {
      term <-
        lapply(term, function (x) x[x %in% prot])
      term <- term[lapply(term,length)>=3]

      if(length(term) == 0) {
        stop("There is no overlap between reference and background")
      }
      ...
      ML.term <-
        max(unlist(lapply(term, function (x) length(x))))
